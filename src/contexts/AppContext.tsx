  ...
  useEffect(() => {
    ...
    const unsubs = subscribeProjectData(projectId, {
      ...
      onTeams: (teams) => setState(s => {
        if ((!teams || !teams.length) && (s.teams?.length || 0) > 0) {
      ...
      onWorkers: (workers) => setState(s => {
        if ((!workers || !workers.length) && (s.workers?.length || 0) > 0) return { ...s, connectionStatus: 'synced' };
        if (workers && workers.length > 0) return { ...s, workers, connectionStatus: 'synced' };
        return { ...s, connectionStatus: 'synced' };
      }),
      onRules: (scoreRules) => setState(s => {
        if ((!scoreRules || !scoreRules.length) && (s.scoreRules?.length || 0) > 0) return { ...s, connectionStatus: 'synced' };
  ...
  const value: AppContextType = {
    ...
    loginWithPassword: async (email, password) => {
      if (isSheetsConfigured()) {
        ...
        return { ok: false, message: sRes.message || 'Email hoặc mật khẩu không đúng' };
      }
      if (isFirebaseConfigured()) {
