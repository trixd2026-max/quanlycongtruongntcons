...
export function ProgressPage() {
  ...
  const qualityTx = transactions.filter(t => t.weekNumber === selectedWeek && !t.isUndone && t.category === 'progress');
  ...
  return (
    <div className="space-y-4">
      <div className="bg-white rounded-xl shadow-sm p-4">
        ...
        <p className="text-sm text-gray-500 mt-1">Tuần {selectedWeek} · {qualityTx.length} sự cố / lỗi chất lượng</p>
      </div>
          ...
          <tbody>
            {qualityTx.map(tx => (
              <tr key={tx.id} className="border-t hover:bg-gray-50">
                ...
                <td className="px-4 py-3 font-medium">{workers.find(w => w.id === tx.workerId)?.fullName}</td>
                <td className="px-4 py-3">{teams.find(t => t.id === tx.teamId)?.name}</td>
            ...
            {qualityTx.length === 0 && <tr><td colSpan={6} className="px-4 py-12 text-center text-gray-400">Không có lỗi chất lượng trong tuần này</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
