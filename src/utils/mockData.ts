...
export function createMockProject(): Project {
  return {
    id: 'project-demo-001',
    name: 'Công trình Nhà máy Điện gió Bình Thuận',
    packageName: 'Gói thầu Xây dựng hạ tầng & Nhà xưởng',
    commanderName: 'Nguyễn Văn Minh',
    phase: '2026 – 2027',
    startDateWeek1: '2026-01-05',
    totalWeeks: 52,
    shiftsPerDay: 2,
...
export function createMockWorkers(projectId: string, teams: Team[]): Worker[] {
  ...
  return data.map((w, i) => ({
    id: `worker-${i + 1}`, projectId, code: w.code, fullName: w.fullName,
    birthDate: `198${5 + (i % 10)}-0${(i % 9) + 1}-15`,
    gender: i % 3 === 0 ? 'female' as const : 'male' as const,
    teamId: teams[w.teamIdx].id, position: w.position,
    phone: `09${String(10000000 + i * 111).slice(0, 8)}`,
    safetyCard: `BHLD-${w.code}`, isActive: true,
...
export function createMockAdmin(projectId: string): AppUser {
  return {
    id: 'user-admin-001', email: 'admin@congtruong.vn', displayName: 'Nguyễn Văn Minh',
    role: 'admin', projectId, isActive: true,
    createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(),
  };
}

export function getGrade(points: number, thresholds: GradeThresholds = DEFAULT_GRADE_THRESHOLDS, hasData = true): string {
